          ...
          <div>
            ...
            <span class="count"
              >{column.intent === "done" && column.hiddenDoneCardCount > 0
                ? `${column.cards.length}/${column.totalCardCount}`
                : column.totalCardCount}</span
            >
          </div>
          {#if column.intent === "done"}
            <div class="done-controls" aria-label="Done card visibility">
              <button
                class:active={doneVisibleLimit === 10}
                type="button"
                onclick={() => onSetDoneVisibleLimit(10)}>10</button
              >
              <button
                class:active={doneVisibleLimit === 20}
                type="button"
                onclick={() => onSetDoneVisibleLimit(20)}>20</button
              >
              <button
                class:active={doneVisibleLimit === "all"}
                type="button"
                onclick={() => onSetDoneVisibleLimit("all")}>All</button
              >
            </div>
          {/if}
        </header>
        ...
        <div class="lane-cards">
          ...
          {#if column.hiddenDoneCardCount > 0}
            <div class="done-hidden-card">
              Showing latest {column.cards.length}. {column.hiddenDoneCardCount} older completed card{column.hiddenDoneCardCount ===
              1
                ? ""
                : "s"} kept in history.
          ...
          {#if column.hiddenLaneCardCount > 0}
            <div class="lane-pagination-card">
              ...
              <div>
                <button type="button" onclick={() => onShowMoreLaneCards(column)}
                  >Show {Math.min(column.hiddenLaneCardCount, 40)} more</button
                >
