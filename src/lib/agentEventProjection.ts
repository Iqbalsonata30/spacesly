...
export function projectAgentTaskSessionEvent(
  ...
  if (event.kind === "lifecycle" && typeof payload.state === "string") {
    ...
    details.push(
      ...
      `- Git/Kubernetes/Bamboo verifiers assigned to exact objectives: ${assignedVerifierCount}`,
      `- Verifier bindings left closed by missing, ambiguous, or unauthorized objective signals: ${unassignedVerifierCount}`,
