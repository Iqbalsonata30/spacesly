...
export function loadUiState(
  ...
  try {
    ...
    const fallbackSession = createWorkspaceChatSession(
      messages.length > 0 ? messages : defaults.chatMessages,
    );
    const legacySession = isWorkspaceChatSession(parsed.workspaceChatSession)
      ? parsed.workspaceChatSession
      : null;
    ...
    return {
      ...
      workspaceShellWorkdir:
        typeof parsed.workspaceShellWorkdir === "string" ? parsed.workspaceShellWorkdir : "",
      workspaceChatMessages: sessionMessages,
      workspaceChatSession: {
        ...activeSession,
        messages: sessionMessages,
      },
      workspaceChatSessions: sessions.map((session) =>
        session.id === activeSession.id ? { ...session, messages: sessionMessages } : session,
      ),
      workspaceChatActiveSessionId: activeSession.id,
      doneVisibleLimit: normalizeDoneVisibleLimit(parsed.doneVisibleLimit),
      workspaceFilesRoot:
        typeof parsed.workspaceFilesRoot === "string" ? parsed.workspaceFilesRoot : "",
      workspaceFilesDirectory:
        typeof parsed.workspaceFilesDirectory === "string" ? parsed.workspaceFilesDirectory : "",
      workspaceFilesActivePath:
        typeof parsed.workspaceFilesActivePath === "string"
          ? parsed.workspaceFilesActivePath
          : null,
    };
  } catch {
    const session = createWorkspaceChatSession(defaults.chatMessages);
